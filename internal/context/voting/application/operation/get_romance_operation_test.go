package operation

import (
	"context"
	"errors"
	"github.com/bmbl-bumble2/recs-votes-storage/internal/testlib/uuidhelper"
	"testing"

	romanceEntity "github.com/bmbl-bumble2/recs-votes-storage/internal/context/voting/domain/romance/entity"
	sharedValueObject "github.com/bmbl-bumble2/recs-votes-storage/internal/context/voting/domain/sharedkernel/valueobject"
	"github.com/bmbl-bumble2/recs-votes-storage/internal/testlib/mocks"
	"github.com/stretchr/testify/suite"
	"go.uber.org/mock/gomock"
)

type GetRomanceOperationUnitTestSuite struct {
	suite.Suite
	voteId       sharedValueObject.VoteId
	ctrl         *gomock.Controller
	romancesRepo *mocks.MockRomancesRepository
	ctx          context.Context
}

func TestGetRomanceOperationUnitSuite(t *testing.T) {
	suite.Run(t, new(GetRomanceOperationUnitTestSuite))
}

func (s *GetRomanceOperationUnitTestSuite) SetupSuite() {
	activeUserId := uuidhelper.NewUUID(s.T())
	peerUserId := uuidhelper.NewUUID(s.T())
	countryId := uint16(11)

	voteId, err := sharedValueObject.NewVoteId(countryId, activeUserId, peerUserId)
	s.Require().NoError(err)
	s.voteId = voteId
	s.ctx = context.Background()
}

func (s *GetRomanceOperationUnitTestSuite) SetupTest() {
	s.ctrl = gomock.NewController(s.T())
	s.romancesRepo = mocks.NewMockRomancesRepository(s.ctrl)
}

func (s *GetRomanceOperationUnitTestSuite) newOperation() *GetRomanceOperation {
	return NewGetRomanceOperation(s.romancesRepo)
}

func (s *GetRomanceOperationUnitTestSuite) TestGetRomanceReturnsError() {
	expectedErr := errors.New("database error")

	s.romancesRepo.EXPECT().
		GetRomance(s.ctx, s.voteId).
		Return(romanceEntity.Romance{}, expectedErr)

	operation := s.newOperation()
	romance, err := operation.Run(s.ctx, s.voteId)

	s.Require().Error(err)
	s.Require().ErrorIs(err, expectedErr)
	s.Require().Equal(romanceEntity.Romance{}, romance)
}

func (s *GetRomanceOperationUnitTestSuite) TestGetRomanceSuccessfully() {
	expectedRomance := romanceEntity.CreateEmptyRomance(s.voteId)

	s.romancesRepo.EXPECT().
		GetRomance(s.ctx, s.voteId).
		Return(expectedRomance, nil)

	operation := s.newOperation()
	romance, err := operation.Run(s.ctx, s.voteId)

	s.Require().NoError(err)
	s.Require().Equal(expectedRomance, romance)
}
